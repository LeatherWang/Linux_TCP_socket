...
int main(int argc, char **argv)
{
 ...
 if (argv[2])
	num = atoi(argv[2]);//进入队列中允许的连接的个数
 else
	num = 3;
	
//理论上建立socket时是指定协议，应该用PF_xxxx，设置地址时应该用AF_xxxx。AF_INET和PF_INET的值是相同的
 ...
 if(argv[3]) 
	server_addr.sin_addr.s_addr = inet_addr(argv[3]);//inet_addr返回的整数形式是网络字节序
 else 
	server_addr.sin_addr.s_addr = INADDR_ANY;//INADDR_ANY就是指定地址为0.0.0.0的地址

 if (bind(sockfd, (struct sockaddr *) &server_addr, sizeof(struct sockaddr)) == -1)//绑定IP地址、端口等信息到socket上，用函数bind()
 {
	perror("bind");
	exit(1);
 }
 ...
 if (listen(sockfd, num) == -1) //num是进入队列中允许的连接的个数
 {
	perror("listen");
	exit(1);
 }
