...
int main()
{
   ...
   while(1)
   {
	 ...
	 while(1)
     {
     	if(link_state == 0)
     		break;
     	bzero(send_buf, send_MAXBUF + 1);
     	stream = popen( "sudo iw dev wlan0 scan | egrep '^BSS|SSID|signal'", "r" );
     	fread( send_buf, sizeof(char), sizeof(send_buf), stream); //将刚刚FILE* stream的数据流读取到buf中
		//printf("%s",send_buf);
